				...
				if (poll == null) {
					...
					if (subscribed.get()) {
						...
						if (reply instanceof MultiBulkReply) {
							...
							LOG.warn("cl-id={} - unexperted message received from server: ignored/dropped - {}", System.identityHashCode(RedisClient.this),
									reply);
						}
					} else {
						LOG.error("cl-id={} - cannot read redis response (poll is null) but it's not a subscribed socket !",
								System.identityHashCode(RedisClient.this));
					}
				} else {
